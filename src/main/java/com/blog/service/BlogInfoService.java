    ...
    public BlogInfoResp getBlogInfo() {
        try {
            return new BlogInfoResp(
                    ...
                    Objects.requireNonNull(stringRedisTemplate.opsForZSet()
                                    .rangeWithScores(SystemConstants.ARTICLE_VIEW_COUNT, 0, -1))
                            .stream().mapToLong(t -> Objects.requireNonNull(t.getScore()).longValue()).sum());
        } catch (Exception e) {
            throw new RuntimeException(e.getMessage() != null ? e.getMessage() : "博客信息接口异常");
        }
    }
